...
export default function CreateCar() {
    ...
    const handleImageSubmit = (e) => {
        if (files.length > 0 && files.length + formData.imageUrls.length < 11) {
            ...
            setImageUploadError('You can only upload 10 images per car');
            setUploading(false);
        }
    };
    ...
    const handleChange = (e) => {
        if (e.target.id === 'sedan' || e.target.id === 'suv' || e.target.id === 'hatchback' ) {
            setFormData({
               ...formData,
                type: e.target.id
            })
        ...
        // if (e.target.id === 'sedan' || e.target.id === 'suv' || e.target.id === 'hatchback' || e.target.id === 'ev') {
        //     setFormData({
        //         ...formData,
        //         [e.target.id]: e.target.checked
        //     })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            //"+" in above statement to convert string to number
            setLoading(true);
            setError(false);
        ...
        <main className='p-3 max-w-4xl mx-auto'>
            ...
            <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
                <div className='flex flex-col gap-4 flex-1'>
                    ...
                    <input onChange={handleChange} value={formData.dealer} type="text" placeholder='Dealer' className='border p-3 rounded-lg' id='dealer' required />
                    
                    <div className='flex gap-6 flex-wrap'>
                        <div className='flex gap-2'>
                            <input onChange={handleChange} checked={formData.type==='sedan'} type="checkbox" id='sedan' className='w-5' />
                            <span>Sedan</span>
                        </div>
                        <div className='flex gap-2'>
                            <input onChange={handleChange} checked={formData.type==='suv'} type="checkbox" id='suv' className='w-5' />
                            <span>SUV</span>
                        </div>
                        <div className='flex gap-2'>
                            <input onChange={handleChange} checked={formData.type==='hatchback'} type="checkbox" id='hatchback' className='w-5' />
                            <span>HatchBack</span>
                        </div>
                        <div className='flex gap-2'>
                            <input onChange={handleChange} checked={formData.ev} type="checkbox" id='ev' className='w-5' />
                            <span>EV</span>
                        </div> 
                        <div className='flex gap-2'>
                ...
                <div className='flex flex-col flex-1 gap-4'>
                    <p className='font-semibold'>Images:
                        <span className='font-normal text-gray-600 ml-2'>The first image will be cover (max 10)</span>
                    </p>
                    ...
                    <p className='text-red-700 text-sm'>{imageUploadError && imageUploadError}</p>
                    {
                        formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
                            <div key={url} className="flex justify-between p-3 border items-center">
                                <img src={url} alt="car image" className='w-25 h-25 object-contain rounded-lg' />
                    ...
                    <button disabled={loading || uploading} className='p-3 bg-slate-700 text-white rounded-lg uppercase opacity-95 disabled:opacity-80'>
